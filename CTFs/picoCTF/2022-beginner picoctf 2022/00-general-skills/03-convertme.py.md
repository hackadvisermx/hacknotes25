...
```
castr-picoctf@webshell:~$ wget https://artifacts.picoctf.net/c/24/convertme.py

python convertme.py 
If 54 is in decimal base, what is it in binary base?
Answer: 
```
- Si tenemos el codigo fuente, podemos hacer un bypass para no tener que reponder las preguntas y obtener la bandera

Codigo fuente original

```python
castr-picoctf@webshell:~$ cat convertme.py 
